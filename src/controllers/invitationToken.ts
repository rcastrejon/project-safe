...
export const invitationTokenController = new Elysia({
  prefix: "/invitationToken",
})
  ...
  .get("/user/:userId", async ({ params: { userId }, error }) => {
    const token =
      await InvitationTokenService.getInvitationTokenByUserId(userId);
    if (!token) return error(404, { error: "Token not found" });
    return { token };
  })
  .post(
    ...
    async ({ body: { userId } }) => {
      try {
        const { token } =
          await InvitationTokenService.createInvitationToken(userId);
        return { token };
      } catch (e) {
        return { error: e };
      }
  ...
  .delete("/:id", async ({ params: { id }, error }) => {
    const deletedToken =
      await InvitationTokenService.deleteInvitationTokenById(id);
